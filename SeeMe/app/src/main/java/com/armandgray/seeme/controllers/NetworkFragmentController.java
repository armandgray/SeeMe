...
public class NetworkFragmentController implements NetworkFragment.NetworkController {
    ...
    public void handleHttpResponse(String response, Parcelable[] arrayExtra, RecyclerView rvNetwork) {
        if (response != null && !response.equals("") && Arrays.asList(responseArray).contains(response)) {
            ...
            updateRvNetworkData(rvNetwork, (User[]) arrayExtra);
        }
    }

    @Override
    public void updateRvNetworkData(RecyclerView rvNetwork, final User[] networkArray) {
        ArrayList<User> list = new ArrayList<>();
        Collections.addAll(list, networkArray);
        UserRVAdapter adapter = new UserRVAdapter(list, true);
        rvNetwork.swapAdapter(adapter, false);

        addRvItemClickListener(rvNetwork, adapter);
    }

    private void addRvItemClickListener(RecyclerView rvNetwork, final UserRVAdapter adapter) {
